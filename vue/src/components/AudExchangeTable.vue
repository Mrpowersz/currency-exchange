<template>
  <div class="container">
    <p>1 EUR to AUD Exchange Rate</p>
    ...
    <div class="table-container">
      ...
      <div class="summary">
        <div class="min-max-container">
          <p class="rate-info"> Minimum: {{ minRate }} AUD, Maximum: {{ maxRate }} AUD</p>
          <p class="rate-info"> Average: {{ avgRate }} AUD </p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    minRate() {
      return Math.min(...this.exchangeRates.map(rate => rate.aud));
    },
    maxRate() {
      return Math.max(...this.exchangeRates.map(rate => rate.aud));
    },
    avgRate() {
      const total = this.exchangeRates.reduce((sum, rate) => sum + rate.aud, 0);
  ...
  methods: {
    async fetchExchangeRates() {
      try {
        const response = await axios.get('http://localhost:3000/exchange-rates?currency=aud');
